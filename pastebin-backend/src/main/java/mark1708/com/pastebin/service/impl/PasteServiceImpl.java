...
public class PasteServiceImpl implements PasteService {
    ...
    public Paste getPasteByHash(String hash) {
        return pasteRepository
                .findById(hash)
                .orElseThrow(() -> new ResourceNotFoundException(ResourceType.PASTE, QueryType.HASH, hash));
    }

    @Override
    @Transactional
    public Paste create(CreatePasteDto createPasteDto) {
        List<Tag> tags = createPasteDto.getTags().stream()
                .map(tagName -> tagService.findByName(tagName).orElseGet(() -> tagService.save(tagName)))
                .toList();
        ...
        return pasteRepository.saveAndFlush(Paste.builder()
                .hash(hash)
    ...
    private Instant getExpirationDateTime(Instant createdAt, String expiration) {
        ...
        switch (expiration) {
            ...
            case "1M" ->
                expirationAt.atZone(ZoneId.systemDefault()).plusMonths(1).toInstant();
            case "6M" -> expirationAt.plus(6, ChronoUnit.MONTHS);
            default -> {
                SimpleDateFormat validExpiresFormatter = new SimpleDateFormat("yyyy-MM-dd");
                try {
                    expirationAt = validExpiresFormatter.parse(expiration).toInstant();
